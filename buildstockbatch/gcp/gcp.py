...
This implementation tries to match the structure of `../aws/aws.py` as much as possible in order to
make it easier to refactor these two (or three, with Eagle) to share code later.
...
class GcpBatch(DockerBatchBase):
    ...
    def registry_url(self):
        ...
        # return f"https://{self.region}-docker.pkg.dev"
...
def main():
    ...
    if 'GCP_BATCH_JOB_ARRAY_INDEX' in os.environ:
        ...
        s3_bucket = os.environ['GCS_BUCKET']
        s3_prefix = os.environ['GCS_PREFIX']
        job_name = os.environ['JOB_NAME']
        region = os.environ['REGION']
        GcpBatch.run_job(job_id, s3_bucket, s3_prefix, job_name, region)
    else:
