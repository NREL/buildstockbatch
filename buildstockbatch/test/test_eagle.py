...
def test_hpc_run_building(mock_subprocess, monkeypatch, basic_residential_project_file):
    ...
    with patch.object(EagleBatch, "weather_dir", None), patch.object(
        ...
        EagleBatch, "make_sim_dir", return_value=("bldg0000001up00", sim_path)
    ), patch.object(
...
def test_user_cli(
    ...
    user_cli(argv)
    mock_subprocess.run.assert_called_once()
    eagle_sh = os.path.abspath(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "eagle.sh")
    )
    ...
    assert (
        ...
        in mock_subprocess.run.call_args[0][0]
    )
...
def test_queue_jobs_minutes_per_sim(
    mocker, basic_residential_project_file, monkeypatch
):
...
def test_run_building_process(mocker, basic_residential_project_file):
    ...
    mocker.patch.object(
        EagleBatch, "local_buildstock_dir", results_dir / "local_buildstock_dir"
    )
    mocker.patch.object(
        EagleBatch, "local_weather_dir", results_dir / "local_weather_dir"
    )
    ...
    sampler_prop_mock = mocker.patch.object(
        EagleBatch, "sampler", new_callable=mocker.PropertyMock
    )
...
def test_rerun_failed_jobs(mocker, basic_residential_project_file):
    ...
    queue_post_processing_mocker = mocker.patch.object(
        EagleBatch, "queue_post_processing"
    )
