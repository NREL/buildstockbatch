...
'''
    ...
    def validate_weather_files(project_file, buildstock_csv_filename, weather_dir):
        ...
        args = BuildStockBatchBase.get_measure_args_from_option_names(options_lookup_path, location_options['Location'].tolist(), 'Location', 'weather_file_name')
    ...
    @staticmethod
    def get_measure_args_from_option_names(lookup_file, option_names, parameter_name, argument_name, runner = None):
        found_options = {}
        ...
        for option_name in option_names:
            if not found_options[option_name]:
                logger.error(f'Could not find parameter {str(parameter_name)} and option {str(option_name)} in {str(lookup_file)}.')
