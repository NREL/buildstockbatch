    ...
    def process_results(self, skip_combine=False, force_upload=False):
        ...
        if self.cfg["workflow_generator"]["type"] == "residential_hpxml":
            if (
                ...
                in self.cfg["workflow_generator"]["args"].keys()
            ):
                if (
                    "timeseries_frequency"
                    in self.cfg["workflow_generator"]["args"][
                        "simulation_output_report"
                    ].keys()
                ):
                    do_timeseries = (
                        self.cfg["workflow_generator"]["args"][
                            "simulation_output_report"
                        ]["timeseries_frequency"]
                        != "none"
                    )
            ...
            do_timeseries = (
                "timeseries_csv_export" in self.cfg["workflow_generator"]["args"].keys()
            )
        ...
        if "s3" in aws_conf or force_upload:
            ...
            if "athena" in aws_conf:
                postprocessing.create_athena_tables(
                    aws_conf, os.path.basename(self.output_dir), s3_bucket, s3_prefix
                )

        keep_individual_timeseries = self.cfg.get("postprocessing", {}).get(
            "keep_individual_timeseries", False
        )
        postprocessing.remove_intermediate_files(
            fs, self.results_dir, keep_individual_timeseries
        )
