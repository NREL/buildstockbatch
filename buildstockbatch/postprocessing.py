...
def clean_up_results_df(df, cfg, keep_upgrade_id=False):
    ...
    for col in ("started_at", "completed_at"):
        if col in results_df.columns:
            results_df[col] = pd.to_datetime(
                results_df[col], format="%Y%m%dT%H%M%SZ"
            ).astype(pd.ArrowDtype(pa.timestamp("s")))
...
def combine_results(fs, results_dir, cfg, do_timeseries=True):
    ...
    for upgrade_id in upgrade_list:
        ...
        if do_timeseries:
            ...
            try:
                ts_filenames = [
                    ts_upgrade_path + ts_filename
                    for ts_filename in fs.ls(ts_upgrade_path)
                ]
                ...
                logger.warning(
                    f"Listing '{ts_upgrade_path}' failed. Skipping this upgrade."
                )
                continue
            ts_bldg_ids = [
...
def create_athena_tables(aws_conf, tbl_prefix, s3_bucket, s3_prefix):
    ...
    try:
        ...
        logger.error(
            f"Could not delete crawler {crawler_name}. Please delete it manually from the AWS console."
        )
        raise error
